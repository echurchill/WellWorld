...
public class RealisticMoonWell extends WellArchetype {
	...
	public void generateChunk(InitialBlocks chunk, int chunkX, int chunkZ) {
		for (int x = 0; x < 16; x++) {
			for (int z = 0; z < 16; z++) {
				int height = NoiseGenerator
						.floor(generator.noise(getNoiseValue(chunkX, x) * 0.0625, getNoiseValue(chunkZ, z) * 0.0625)
								* surfaceVariance)
						+ surfaceAt;
		...
		if (random.nextInt(100) <= craterChance) {
			...
			if (meteoriteRadius > 0) {
				...
				if (meteoriteRadius == 1) {
					...
					drawSolidSphere(world, chunk, meteoriteX, meteoriteY, meteoriteZ, meteoriteRadius,
							meteoriteMaterial);
				}
			}
		}
