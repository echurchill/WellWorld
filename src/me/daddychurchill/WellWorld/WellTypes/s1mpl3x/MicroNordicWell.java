...
public class MicroNordicWell extends WellArchetype {
	...
	public void generateChunk(InitialBlocks chunk, int chunkX, int chunkZ) {
		for (int x = 0; x < 16; x++) {
			for (int z = 0; z < 16; z++) {
				// double noiz = 25+noisegen.noise((x+x_chunk*16)/100.0f, (z+z_chunk*16)/100.0f,
				// 0.5, 0.5)*15;
				double noiz = baseHeight
						+ noisegen.noise((x + chunkX * 16) / 100.0f, (z + chunkZ * 16) / 100.0f, 0.5, 0.5)
								* baseVariability;
