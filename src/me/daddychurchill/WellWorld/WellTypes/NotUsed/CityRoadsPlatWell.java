...
public class CityRoadsPlatWell extends WellArchetype {
	...
	private SimplexNoiseGenerator noiseUrbanMaterial; // which building material set to use (if an adjacent chunk is
														// similar then join them)
	...
	public boolean isRoad(int x, int z) {
		...
		if (roadExists) {
			...
			if (checkRoadCoordinate(x) && checkRoadCoordinate(z)) {
				...
				roadExists = isRoad(x - 1, z) || isRoad(x + 1, z) || isRoad(x, z - 1) || isRoad(x, z + 1);
				...
				// overwater?
				if (isBridge)
					roadExists = (previousNoise + nextNoise) > (threshholdBridge + lengthNoise); // longer bridges are
																									// "harder"
				else
